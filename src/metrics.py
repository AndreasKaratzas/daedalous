...
class MetricLogger():
    ...
    def record(self, episode, step):
        mean_ep_reward = numpy.round(numpy.mean(self.ep_rewards[-10:]), 3)
        mean_ep_length = numpy.round(numpy.mean(self.ep_lengths[-10:]), 3)
        mean_ep_loss = numpy.round(numpy.mean(self.ep_avg_losses[-10:]), 3)
        ...
        print(
            f"Episode {episode:>7} - "
            f"Step {step:>10} - "
            f"Min Return {numpy.round(numpy.min(self.ep_rewards[-10:]), 3):>8.3f} - "
            f"Max Return {numpy.round(numpy.max(self.ep_rewards[-10:]), 3):>8.3f} - "
        ...
        with open(self.save_log, "a") as f:
            f.write(
                ...
                f"{numpy.round(numpy.min(self.ep_rewards[-10:]), 3):15.3f}{numpy.round(numpy.max(self.ep_rewards[-10:]), 3):15.3f}"
